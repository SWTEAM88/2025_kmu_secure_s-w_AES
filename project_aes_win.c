...
static void construct_counter_block(byte counter_block[AES_BLOCK_SIZE], const byte* nonce, uint64_t counter) {
    ...
    // 뒤 8바이트 = counter (little endian으로 처리되어 저장되므로 그대로 사용)
    // 주의: uint64_t를 바이트 배열로 변환할 때, 인덱스 0부터 7까지 순서대로 8바이트를 저장하는 방식으로 작성 (Little Endian)
    for (int i = 0; i < 8; i++) {
...
int aes_keyexpansion(const byte* key, word* w, size_t key_len) {
    ...
    // 처음 Nk개의 입력 키를 그대로 복사 (Big Endian으로 저장)
    for (int i = 0; i < Nk; i++) {
...
void aes_decrypt(byte input[AES_BLOCK_SIZE], byte output[AES_BLOCK_SIZE], const word* w, int Nr) {
    ...
    for (int round = Nr - 1; round >= 1; round--) {
        ...
        aes_addroundkey(w, state, round); // 라운드 키 순서가 복호화와 반대
...
void AES_ECB(const byte* input, byte* output, size_t length,
    ...
    // 나머지 데이터가 16바이트 미만인 경우 (패딩 처리 그대로 유지)
...
//// SHA-256 상수 (처음 64개 소수들의 세제곱근의 소수 부분)
